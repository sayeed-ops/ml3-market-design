...
**Projects** workspace (brand as a scope switcher) → project detail, and the **Account** area. The
universal sidebar links the areas together, and the sidebar user card opens the account menu.
...
- `market-ml3/` — **Market**: `index.html` (market table), `details.html` + `details.css`
  (media details), `drafts.html`, `checkout.html` + `checkout.css` (cart). Owns the shared shell
  styles (`styles.css`) + `pager.js` + logos that the other areas reference.
